...
d_user <- d_user %>% left_join(gradients, by = c("group" = "group"))
```
...
d_user %>%
  ggplot(aes(x = twlz_member_group, y = edchatde_member_group, color = Freq)) +
  geom_jitter(show.legend = TRUE) +
  # geom_tile() +
  scale_color_viridis_c(breaks = c(-6, -3, 0, 3, 6), labels = scales::label_number(accuracy = 2)) +
  # scale_color_continuous(type = "viridis") +
  # guide_colorsteps(aes(nbin = 6)) +
  papaja::theme_apa(box = TRUE) +
  labs(x = "TWLZ Member Group", y = "#EdChatDE Member Group") +
  # theme(axis.title.y = element_text(margin = margin(r = -.1, unit = "pt"))) +
  scale_x_discrete(labels = scales::wrap_format(10)) +
...
d_user_switch <- d_user_switch %>%
  mutate(
    edchatde_member_group_f = factor(edchatde_member_group, level = c("innovators", "early adopters", "early majority", "late majority", "laggards"))
  )

g_switch <- d_user_switch %>%
  ggplot(aes(
    x = edchatde_member_group_f,
    y = user_switch_time_standard,
    fill = above_median_lifespan
  )) +
  scale_x_discrete(labels = scales::wrap_format(10)) +
  ...
  papaja::theme_apa(box = TRUE) +
  labs(x = "#EdChatDE Member Group", y = "User Switch Time", fill = "Median Lifespan") +
  theme(legend.position = "bottom") +
  theme(legend.margin = margin(t = -5, unit = "pt"))
g_switch

# ggsave(plot = g_switch, filename = "switch_explain.png", height = 4, width = 6)
```
