...
g_users <- time_stat_users %>%
  ...
  scale_x_continuous(labels = as.character(years), breaks = years) +
  papaja::theme_apa(box = TRUE) +
  labs(x = "Year", y = expression(paste("Users ", (log[10]))), color = "Community") +
...
# Fit + Search
m <- lm(user_switch_time_standard ~ . + 0, d_model)
summary(m)
...
mm_stats %>%
  kbl(
    ...
    col.names = c("Effect", "Estimate", "SE", "p"),
    digits = c(0, 2, 2, 3)
  )
```
